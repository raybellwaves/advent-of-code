...
TL;DR: sum consecutive items in a list, find the max
...
TL;DR: sum consecutive items in a list, find the top 3 and sum
"""
...
for i in lines:
    if i == "":
        ...
        elf_calorie_count += int(i)
        d[elf_number] = elf_calorie_count
    if elf_calorie_count > max_elf_calorie_count:
        max_elf_calorie_count = elf_calorie_count
print(max_elf_calorie_count)
print(sum(sorted(list(d.values()))[-3:]))

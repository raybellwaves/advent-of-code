...
the two compartments. The Elf that did the packing failed to follow this rule for
...
which means its first compartment
contains the items vJrwpWtwJgWr,
...
list is all items in rucsack
each item type is either a (small) or A (big)
a line is list of items of for a rucksack
first half - first component; second half - second component
a type which is in both compartments is p (16), second is L (38), thrid is P (22), v (22), t (20), s (19) - sum 157
a-z 1-26; A-Z 27-52
...
tell which item type is the right one is by finding the one item type that is common between all three
Elves in each group.
...
TL; DR:
badge B means three elfs (group) all have B
find the item comment between the elfes
...
"""
d = {}
for i, j in enumerate(string.ascii_lowercase + string.ascii_uppercase):
    d[j] = i + 1
...
for i in range(0, len(inputs_splitted), 3):
    e1 = inputs_splitted[i]
    e2 = inputs_splitted[i + 1]
    e3 = inputs_splitted[i + 2]
    c = set(e1).intersection(set(e2)).intersection(set(e3))
    _l.append(d["".join(c)])
sum(_l)
